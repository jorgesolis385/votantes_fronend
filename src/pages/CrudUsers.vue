		...
		<div class="col-12">
			<div class="card">
				...
				<DataTable ref="dt" :value="entitys" v-model:selection="selectedEntitys" dataKey="id" :paginator="true" :rows="10" :filters="filters"
					...
					<template #header>
						<div class="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
							<h5 class="m-0">Administrar Userios</h5>
				...
				<Dialog v-model:visible="entityDialog" :style="{width: '450px'}" header="Información Tenans" :modal="true" class="p-fluid">
					<TabView>
					<TabPanel header="Informacion Usuario">
						<div class="formgrid grid">
						<div class="field col">
							<label for="name">Nombres</label>
							<InputText id="name" v-model="entity.firstName" />
						</div>
						<div class="field col">
							<label for="type">Apellidos</label>
							<InputText id="type" v-model="entity.lastName" />
						</div>
					</div>
					<div class="field">
						<label for="name">Nombre usuario</label>
						<InputText id="name" v-model.trim="entity.username" required="true" autofocus :class="{'p-invalid': submitted && !entity.username}" />
						<small class="p-invalid" v-if="submitted && !entity.username">nombre de usuario es requerido.</small>
					</div>
					<div class="field">
						<label for="name">Correo</label>
						<InputText id="name" v-model.trim="entity.email" required="true" autofocus :class="{'p-invalid': submitted && !entity.email}" />
						<small class="p-invalid" v-if="submitted && !entity.email">El correo es requerido.</small>
					</div>					
					<div class="field">
						<label for="state">Tenants</label>
						<Dropdown id="defaultTenant" v-model="entity.defaultTenant" :options="dropdownItems" optionLabel="name" optionValue="id" placeholder="Escoja un tenants"></Dropdown>
					</div>
					
					</TabPanel>
					<TabPanel header="Seguridad" :disabled="!entity.id">
				...
				<Dialog v-model:visible="deleteEntityDialog" :style="{width: '450px'}" header="Confirm" :modal="true">
					<div class="flex align-items-center justify-content-center">
						...
						<span v-if="entity">Desea eliminar el User <b>{{entity.name}}</b>?</span>
					</div>
					<template #footer>
...
export default {
	...
	mounted() { 
		//this.userService.getTenants().then(data => this.entitys = data);
		this.loadEntitys();
	...
	methods: {		
		openNew() {
			...
			this.userService.getTenants().then(data => this.dropdownItems = data );			
		},
		hideDialog() {
		...
		saveEntity() {
			...
			if (this.entity.username.trim()) {
				if (this.entity.id) {	
					this.entity.middleName=this.entity.firstName+ " "+this.entity.lastName;
					this.entity.defaultTenant = this.entity.defaultTenant.code								
					this.userService.updateUser(this.entity).then(()=>{
					this.$toast.add({severity:'success', summary: 'Successful', detail: 'User Actualizado', life: 3000});
					}).catch((data)=>{
						console.log(data.error);
						this.$toast.add({severity:'error', summary: 'Error Actualizando', detail: 'Error User Actualizado', life: 3000});	
					});
					this.loadEntitys();
				}else{					
					this.entity.middleName=this.entity.firstName+ " "+this.entity.lastName;
					this.entity.defaultTenant = this.entity.defaultTenant.code
					this.userService.createdUser(this.entity).then(()=>{
					this.$toast.add({severity:'success', summary: 'Successful', detail: 'Usuario Creado con exito', life: 3000});
					}).catch((data)=>{
						console.log(data);
						this.$toast.add({severity:'error', summary: 'Error Creando ', detail: 'Error al creal el usuario', life: 3000});	
					});
				this.loadEntitys();
				}	
			}
		...
		deleteEntity() {
			this.userService.deleteUser(this.entity).then(()=>{
					this.$toast.add({severity:'success', summary: 'Successful', detail: 'User Eliminado', life: 3000});
						this.loadEntitys();
					}).catch(()=>{						
						this.$toast.add({severity:'error', summary: 'Error Eliminando ', detail: 'Error User Actualizado', life: 3000});	
					})
		...
		loadEntitys(){
			this.userService.getUsers().then(data => this.entitys = data);			
		},
