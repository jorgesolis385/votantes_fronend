			...
			<div class="card">
				...
				<DataTable ref="dt" :value="entitys" v-model:selection="selectedEntitys" dataKey="id" :paginator="true" :rows="10" :filters="filters"
					...
					<Column field="address1" header="Address1" :sortable="true">
						<template #body="slotProps">
							<span class="p-column-title">Modificado</span>
							{{slotProps.data.modified_on}}
						</template>
					</Column>				
				...
				<Dialog v-model:visible="entityDialog" :style="{width: '650px'}" header="Información Tenans" :modal="true" class="p-fluid">				
					<div class="formgrid grid">
						<div class="field col">
							<label for="name">Nombre</label>
							<InputText id="name" v-model="entity.name" />
						</div>						
					</div>
					<div class="field">
						<label for="state">Permisos</label>
						<PickList v-model="entity.permissions"  dataKey="permissions" v-model:modelValue="entity.permissions">
				...
				<Dialog v-model:visible="deleteEntityDialog" :style="{width: '450px'}" header="Confirm" :modal="true">
					<div class="flex align-items-center justify-content-center">
						...
						<span v-if="entity">Desea eliminar el Rol <b>{{entity.name}}</b>?</span>
					</div>
					<template #footer>
				...
				<Dialog v-model:visible="deleteEntitysDialog" :style="{width: '450px'}" header="Confirm" :modal="true">
					<div class="flex align-items-center justify-content-center">
						<i class="pi pi-exclamation-triangle mr-3" style="font-size: 2rem" />
						<span v-if="entity">Desea eliminar los roles?</span>
					</div>
					<template #footer>
...
export default {
	...
	mounted() { 
		//this.rolService.getPermissions().then(data => this.entitys = data);
		this.loadEntitys();
	},
	methods: {		
		openNew() {
			...
			this.rolService.getPermissions().then(res => this.permissions = [res.map(function(x) {return JSON.parse('{"name": "'+x.name+'","code":"'+x.name+'"}'); }),[]]   ); 		
		},
		hideDialog() {
		...
		/*	if (this.entity.rolname.trim()) {
				if (this.entity.id) {	
					...
					this.rolService.updateRol(this.entity).then(()=>{
					this.$toast.add({severity:'success', summary: 'Successful', detail: 'rol Actualizado', life: 3000});
					}).catch((data)=>{
						console.log(data.error);
						this.$toast.add({severity:'error', summary: 'Error Actualizando', detail: 'Error rol Actualizado', life: 3000});	
					});
					this.loadEntitys();
				}else{		
					this.rolService.createdRol(this.entity).then(()=>{
		...
		editEntity(entity){	
	//	this.rolService.getPermissions().then(res => this.permissions = [res.map(function(x) {return JSON.parse('{"name": "'+x.name+'","code":"'+x.name+'"}'); }),[]]   );
		entity.permissions = [entity.permissions.map(function(x) {return JSON.parse('{"name": "'+x+'","code":"'+x+'"}'); })
		,[]];
		...
		deleteEntity() {
			this.RolService.deleteRol(this.entity).then(()=>{
					this.$toast.add({severity:'success', summary: 'Successful', detail: 'Rol Eliminado', life: 3000});
						this.loadEntitys();
					}).catch(()=>{						
						this.$toast.add({severity:'error', summary: 'Error Eliminando ', detail: 'Error Rol Actualizado', life: 3000});	
					})
		...
		loadEntitys(){
			this.rolService.getRols().then(data => this.entitys = data);
		},
